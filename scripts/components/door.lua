...
--注意到跟随玩家的生物在玩家进出房门时，并不会立即随着玩家传送，而是需要脱离加载，因此写下这个补丁，让跟随玩家的生物随着玩家一起在进出房门时被传送，模仿了原版虫洞的处理方式
local function ShouldTeleportFollower(follower)
    ...
    if follower:HasTag("beefalo") and follower:HasTag("hitched") then
        return false
    end
...
local function TeleportFollower(follower, x, y, z)
    if follower.Physics ~= nil then
        follower.Physics:Teleport(x, y, z)
    elseif follower.Transform ~= nil then
        ...
        if player.components.leader ~= nil then
            for follower, v in pairs(player.components.leader.followers) do
                if ShouldTeleportFollower(follower) then
                    TeleportFollower(follower, x, y, z)
                end
            end
        end
