...
local function WallPlaceTest(inst, distance)
    ...
    if current_interior then
        ...
        local ents = TheSim:FindEntities(newpt.x, 0, newpt.z, 5.5, {"fullwallsection"})
        if #ents > 0 then
            canbuild = false
        end

        local dist = distance or 2.2
        ents = TheSim:FindEntities(newpt.x, 0, newpt.z, dist, {"wallsection"})
