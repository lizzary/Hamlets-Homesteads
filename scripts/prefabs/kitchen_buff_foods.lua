...
local function healthvalue_with_kitchen_buff(health)
    if health < 0 then
        return 0
    end
    return health + 50
...
local function sanityvalue_with_kitchen_buff(sanity)
    if sanity < 0 then
        return 0
    end
    return sanity + 15
...
local function hungervalue_with_kitchen_buff(hunger)
    if hunger < 0 then
        return 0
    end
    return hunger * 1.5
...
local function perishtime_with_kitchen_buff(time)
    return time * 1.5
end
local function OnSave(inst, data)
    data.healthvalue = inst.components.edible.healthvalue
    data.hungervalue = inst.components.edible.hungervalue
    data.sanityvalue = inst.components.edible.sanityvalue
end

local function OnLoad(inst, data)
    if data then
        if data.healthvalue then
            inst.components.edible.healthvalue = data.healthvalue
        end
        if data.hungervalue then
            inst.components.edible.hungervalue = data.hungervalue
        end
        if data.sanityvalue then
...
local function MakePreparedFoodWithBuff(data)
    ...
    local foodassets =
    {
        ...
        Asset("INV_IMAGE", original_name), -- 使用原始名称获取图片
    }
    ...
    local function DisplayNameFn(inst)
        if data.spice ~= nil then
            ...
            return subfmt(STRINGS.NAMES[data.spice.."_FOOD"], {
                food = STRINGS.NAMES[string.upper(original_basename)]
            }) .. STRINGS.BUFF.KITCHEN_BUFF
        else
            -- 普通料理的显示名称
            return STRINGS.NAMES[string.upper(original_basename)] .. STRINGS.BUFF.KITCHEN_BUFF
        end
    end
    ...
    local function fn()
        ...
        inst.AnimState:OverrideSymbol("swap_food", data.overridebuild or "cook_pot_food", original_basename)
        inst.scrapbook_overridedata = {"swap_food", data.overridebuild or "cook_pot_food", original_basename}
        ...
        -- 设置原始名称以便引用
        inst.original_prefab_name = original_name
        ...
        inst.food_basename = original_basename

        inst:AddComponent("edible")
        inst.components.edible.healthvalue = healthvalue_with_kitchen_buff(data.health)
        inst.components.edible.hungervalue = hungervalue_with_kitchen_buff(data.hunger)
        inst.components.edible.foodtype = data.foodtype or FOODTYPE.GENERIC
        inst.components.edible.secondaryfoodtype = data.secondaryfoodtype or nil
        inst.components.edible.sanityvalue = sanityvalue_with_kitchen_buff(data.sanity or 0)
        ...
        -- 使用原始名称获取图像（避免找不到带后缀的图片）
        if spicename ~= nil then
            inst.components.inventoryitem:ChangeImageName(spicename.."_over")
        elseif original_basename ~= nil then
            inst.components.inventoryitem:ChangeImageName(original_basename)
        end

        inst:AddComponent("stackable")
        inst.components.stackable.maxsize = TUNING.STACK_SIZE_SMALLITEM
        -- 确保不会和原版料理堆叠
        --inst.components.stackable.unique_prefab = true

        if data.perishtime ~= nil and data.perishtime > 0 then
            inst:AddComponent("perishable")
            inst.components.perishable:SetPerishTime(perishtime_with_kitchen_buff(data.perishtime))
