...
local function CreateFrontVisual(inst, name, anim_def) -- 柜子前方用于遮挡的图
...
local function CreateLockVisual(inst, name, anim_def) -- 柜子前方用于遮挡的图
...
local function MakeShelf(name, physics_round, anim_def, slot_symbol_prefix, on_robbed, master_postinit)
    local function fn()
        ...
        if physics_round then
            ...
            -- MakeInteriorPhysics(inst, 2, 1, 0.5) -- 暂时取消这些贴边物体的碰撞体和寻路，以减少卡位的可能
        ...
        inst:DoStaticTaskInTime(0, function()
            if inst.frontvisual then
                inst.frontvisual.Follower:FollowSymbol(inst.GUID, nil, 0, 0, 0.0015) -- 毫无疑问，这是为了解决层级bug的屎山，因为有时SetFinalOffset会失效（特别是在�?点特别远的位置）
            end
            if inst.lockvisual then
                inst.lockvisual.Follower:FollowSymbol(inst.GUID, nil, 0, 0, 0.002) -- 毫无疑问，这是为了解决层级bug的屎山，因为有时SetFinalOffset会失效（特别是在�?点特别远的位置）
            end
        end)
